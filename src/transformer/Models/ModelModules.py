...
class ChordGeneratorTransformer(nn.Module):
    ...
    def forward(self, pitch_tokens, duration_tokens):
        x = self.embedding(pitch_tokens, duration_tokens)
        ...
        decoded = self.decoder(x, x, tgt_mask=mask)

        pitch_out = self.output_pitch(decoded)
        duration_out = self.output_duration(decoded)
